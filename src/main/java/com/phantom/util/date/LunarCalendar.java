...
public class LunarCalendar {
    private final Logger log = Logger.getLogger(LunarCalendar.class);
    ...
    private void init(long timeInMillis) {
        if (timeInMillis > minTimeInMillis && timeInMillis < maxTimeInMillis) {
            ...
            this.maxDayInMonth = daysInLunarMonth != 0 ? daysInLunarMonth : LunarCalendar.getLunarMonthDays(this.lunarYear, lunarMonth);
    ...
    private static int getLunarYearDays(int lunarYear) {
        ...
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            // 每个大月累加一天
            daysInLunarYear += ((LunarCalendar.lunarInfo[lunarYear - 1900] & i) != 0) ? 1 : 0;
        }
        // 加上闰月天数
    ...
    private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
        ...
        int daysInLunarMonth = ((LunarCalendar.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30 : 29;
